...
| CT-SEL-014 | Ressarcimento de PIS/Pasep e Cofins nao cumulativos | DCOMP posterior a PER e marco de 361 dias | `per_dcomp-web_-ressarcimento-de-pis_pasep-e-cofins-nao-cumulativos.pdf`, demonstrativo do credito | DCOMP posterior a PER com data de transmissao do pedido de ressarcimento original, valor do pedido/saldo original remanescente e data de entrega da DCOMP | SELIC desde o mes seguinte ao do 361 dia contado da transmissao do PER original ate o mes anterior a entrega da DCOMP, mais 1%; credito original utilizado = total dos debitos / `(1 + taxa Selic)` | Futuro `SelicService`, `CreditoRulesService`, `CalculoService.ts` | Em especificacao |
...
| CT-SEL-016 | Ressarcimento de IPI | SELIC do credito de IPI, sem apuracao operacional completa | IN RFB n. 2.055/2021, arts. 148 e seguintes; `per_dcomp-web_ressarcimento-de-ipi.pdf` apenas como contexto de tipo de credito/PER previo | Credito de IPI com valor original/base importada ou informada, PER/DCOMP de referencia e datas necessarias para valoracao | Calcular apenas a SELIC do credito conforme IN RFB n. 2.055/2021; nao implementar RAIPI, apuracao trimestral, estornos ou demais regras operacionais extensas do roteiro de IPI; nao usar analogia de PIS/Cofins sem fonte | Futuro `SelicService`, `CalculoService.ts` | Em especificacao |
