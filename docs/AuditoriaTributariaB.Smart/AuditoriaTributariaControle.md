...
- Status geral: Fase 3 parcial implementada e revalidada
...
- Implementacao Fase 3 parcial: `SelicService` calcula resultado rastreavel em modo separado, com `normativo` apenas quando dados e taxa forem suficientes; credito judicial sem componentes retorna `dados_insuficientes`; ausencia de taxa pode retornar `estimativa_historica` como fallback identificado.
- Proxima decisao recomendada: ampliar Fase 3 para demais tipos/metadados e, depois, decidir a integracao controlada com `CalculoService` na Fase 4.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AUD-01 | SELIC e atualizacao de creditos | `01-SELICAtualizacaoCreditos.md` | Critica | Revalidado | Validar marco inicial, marco final, acrescimo de 1%, tipo de credito, componentes de credito judicial e uso da tabela SELIC. | Calculo simplificado pode distorcer consumo de credito em simulacoes; regras variam por tipo de credito e, no credito judicial, por componente/forma de atualizacao. | Fase 3 parcial criou `SelicService` rastreavel fora da cascata ativa. Com dados reais importados, saldo negativo IRPJ gera resultado `normativo` quando taxa e dados suficientes sao fornecidos; credito judicial real sem componentes retorna `dados_insuficientes`; ausencia de taxa pode retornar `estimativa_historica` com `fator_historico_identificado`. | Ampliar cobertura por tipo de credito e preparar integracao controlada com `CalculoService` somente quando `statusCalculo = normativo`, mantendo fallback historico identificado. |
...
| AUD-09 | Casos de teste normativos e evidencias | `09-CasosTesteMatrizEvidencias.md` | Critica | Revalidado | Criar matriz de testes por tipo de credito, regra e caso real. | Ajustes futuros sem teste podem quebrar regras tributarias sensiveis. | Automacao atual possui 10 arquivos e 39 testes. Cobre matematica SELIC com taxa injetada, marcos de data, classificacao de credito/status, fixtures FX-SEL-001 a FX-SEL-008, regra de cascata consultiva, guarda de `...Original`, importacao real das planilhas em `Sheets/`, `ImportQualityReport`, preservacao no store e `SelicService` com dados reais. | Manter fixtures como contrato de regressao e ampliar cobertura para pagamento indevido/eSocial, CPIM e art. 152 com PER original quando houver dado real ou complementar rastreavel. |
...
| AUD-11 | Desenho tecnico da implementacao normativa | `11-DesenhoTecnicoImplementacaoNormativa.md` | Critica | Revalidado | Consolidar contratos, camadas, ordem de implementacao, gates e anti-regras antes de alterar codigo. | Implementar regras espalhadas ou sem testes pode contaminar valores originais e misturar calculo normativo com estimativa. | Fase 1 criou contratos/servicos puros; Fase 2 integrou importacao de metadados, qualidade de importacao e classificadores consultivos; Fase 3 parcial criou `SelicService` rastreavel sem recalculo ativo da cascata. | Proxima fase deve ampliar tipos e preparar integracao controlada com `CalculoService`, sem sobrescrever `...Original`. |
