...
- Implementacao Fase 2: `ExcelParser.ts` e modelo `DCOMP` agora preservam metadados opcionais de credito importados, retornam `ImportQualityReport`, anexam classificacao consultiva de credito/status por DCOMP e preservam o relatorio de qualidade no store, sem alterar cascata ativa.
...
- Proxima decisao recomendada: planejar versionamento do estado persistido em IndexedDB e levar o contrato de rastreabilidade para Excel futuro.
...
| AUD-08 | Relatorios PDF/Excel e rastreabilidade | `08-RelatoriosPDFExcelRastreabilidade.md` | Alta | Parcialmente implementado | Garantir que relatorios mostrem original, simulado, delta, status e fonte do calculo. | Relatorio pode ficar denso ou ainda incompleto para DCOMP hipotetica/Excel. | PDF possui premissas/metodologia e, no Passo 1, passou a imprimir origem/metodo/status por valor quando o snapshot possui `rastreabilidadeValores`. | Revisar ergonomia visual do PDF e planejar Excel futuro com o mesmo contrato. |
| AUD-09 | Casos de teste normativos e evidencias | `09-CasosTesteMatrizEvidencias.md` | Critica | Revalidado | Criar matriz de testes por tipo de credito, regra e caso real. | Ajustes futuros sem teste podem quebrar regras tributarias sensiveis. | Automacao atual possui 15 arquivos e 62 testes. Cobre servicos normativos, importacao real, store, rastreabilidade, tooltips, PDF com origem/metodo/status, Worker pipeline e vedacao DCTFWeb consultiva. | Manter fixtures como contrato de regressao e registrar novas evidencias ao expandir Excel, DCOMP hipotetica e schema de snapshot. |
