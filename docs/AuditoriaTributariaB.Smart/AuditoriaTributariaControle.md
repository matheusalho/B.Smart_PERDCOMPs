...
- Status geral: checkpoint `b4c779a` revalidado; Fases 1-4 e melhorias consultivas/UI parcialmente integradas, com limites residuais documentados.
...
- Implementacao atual: Fase 1 autorizada em 2026-06-07 e limitada a infraestrutura de testes, contratos e servicos puros em `src/services/normativo/`, sem integracao ao fluxo ativo.
- Implementacao Fase 2: `ExcelParser.ts` e modelo `DCOMP` agora preservam metadados opcionais de credito importados, retornam `ImportQualityReport`, anexam classificacao consultiva de credito/status por DCOMP e preservam o relatorio de qualidade no store. Em 2026-06-23, a linhagem passou a usar timestamp consistente da aba de debitos como desempate de documentos do mesmo dia.
- Implementacao atual confirmada em codigo: `CalculoService.ts` chama `calcularSelicRastreavel(...)` e usa resultado normativo quando disponivel para DCOMP editada; casos sem dado/taxa suficiente preservam fallback historico identificado.
- Rodada recente `b4c779a`: painel de rastreabilidade, `StatusBadge`, tooltips consultivos, fallback local do Worker, metadados adicionais de importacao, vedacao DCTFWeb consultiva por PA do credito, ajustes de PDF e 14 arquivos/60 testes.
- Passo 1 posterior: `rastreabilidadeValores` em snapshots e origem/metodo/status por valor no PDF; `npm test` aprovado com 15 arquivos/62 testes.
- Relatorio Excel posterior: sete abas auditaveis geradas de `simulacoesSalvas`, perfil visual e-CAC, formatos tipados e evidencias; `Projeção Retificações Cadeias` e a primeira aba e orienta o trabalho de retificacao com valores atuais/corretos.
- Proxima decisao recomendada: planejar versionamento do estado persistido em IndexedDB e ampliar metadados de DCOMP hipotetica.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AUD-01 | SELIC e atualizacao de creditos | `01-SELICAtualizacaoCreditos.md` | Critica | Revalidado / integrado com fallback | Validar marco inicial, marco final, acrescimo de 1%, tipo de credito, componentes de credito judicial e uso da tabela SELIC. | Calculo simplificado ainda pode distorcer cenarios sem dados suficientes; regras variam por tipo de credito e, no credito judicial, por componente/forma de atualizacao. | `SelicService` existe e e chamado por `CalculoService.ts`; resultado `normativo` pode substituir o consumo em DCOMP editada quando dados/taxa sao suficientes; credito judicial sem componentes segue `dados_insuficientes`; ausencia de taxa/dado preserva fallback identificado. | Expandir rastreabilidade por valor/documento e evitar que fallback seja apresentado como normativo. |
...
| AUD-03 | Importacao do relatorio e-CAC e linhagem | `03-ImportacaoRelatorioECACELinhagem.md` | Alta | Revalidado | Auditar parser, normalizacao, datas, agrupamento por cadeia, retificacoes e cancelamentos. | Mudanca de coluna da RFB pode quebrar importacao; erro de linhagem altera cascata inteira; marcos de SELIC podem existir na planilha mas nao no modelo. | Fase 2 implementou `metadadosCreditoImportado`, `ImportQualityReport`, classificacoes consultivas de credito/status por DCOMP e preservacao do relatorio de qualidade no store. Testes usam planilhas reais em `Sheets/`, incluindo a mais recente com cabecalhos oficiais; validam 1472 linhas de processamento, 1443 DCOMPs, 565 cadeias, 4641 debitos carregados e 29 documentos ignorados por ausencia de cadeia. | Prosseguir para Fase 3 com datas/SELIC calculadas em campos separados, sem alterar valores importados nem `...Original`. |
...
| AUD-08 | Relatorios PDF/Excel e rastreabilidade | `08-RelatoriosPDFExcelRastreabilidade.md` | Alta | Implementado com lacunas residuais | Garantir que relatorios mostrem original, simulado, delta, status e fonte do calculo. | Relatorio pode ficar denso ou ainda incompleto para DCOMP hipotetica e snapshots antigos. | PDF possui premissas/metodologia e badges; Excel consolida sete abas e abre com roteiro de retificacoes, pares atuais/corretos, motivos, SELIC, status e evidencias a partir de `simulacoesSalvas`. | Revisar ergonomia visual do PDF e ampliar metadados de DCOMP hipotetica sem reconstruir dados ausentes. |
| AUD-09 | Casos de teste normativos e evidencias | `09-CasosTesteMatrizEvidencias.md` | Critica | Revalidado | Criar matriz de testes por tipo de credito, regra e caso real. | Ajustes futuros sem teste podem quebrar regras tributarias sensiveis. | Automacao atual possui 16 arquivos e 82 testes. Cobre servicos normativos, importacao real, ordenacao por timestamp, store, rastreabilidade, tooltips, PDF, Excel, Worker pipeline e vedacao DCTFWeb consultiva. | Manter fixtures como contrato de regressao e registrar novas evidencias ao expandir DCOMP hipotetica e schema de snapshot. |
| AUD-10 | Base geral PER/DCOMP Web | `10-BaseGeralPERDCOMPWeb.md` | Alta | Solucao proposta | Consolidar orientacoes gerais aplicaveis a multiplos tipos de credito e objetos de auditoria. | Corrigir regras especificas sem base transversal pode gerar inconsistencias entre tipos de credito, debitos, UI e relatorio. | Quatro manuais gerais foram consolidados e conectados a camadas tecnicas: `CreditoRulesService`, `VedacaoCompensacaoService`, `StatusRulesService`, modelo de debitos, origem de valores e relatorios. ACH-025 registra que regras gerais devem ser catalogos consultivos coordenados, nao um unico booleano. | Implementar catalogos consultivos separados para meio cabivel, vedacao de credito, vedacao de debito, status processual, qualidade de importacao e status de calculo SELIC. |
| AUD-11 | Desenho tecnico da implementacao normativa | `11-DesenhoTecnicoImplementacaoNormativa.md` | Critica | Revalidado / parcialmente executado | Consolidar contratos, camadas, ordem de implementacao, gates e anti-regras antes de alterar codigo. | Implementar regras espalhadas ou sem testes pode contaminar valores originais e misturar calculo normativo com estimativa. | Fases 1-4 foram parcialmente executadas: contratos, importacao, classificadores, SELIC, provider, integracao controlada, UI consultiva, PDF inicial e fallback de Worker. | Proxima fase deve consolidar origem/metodo por valor e versionamento de estado, sem sobrescrever `...Original`. |
...
### ACH-025 - Regras gerais precisam ser catalogos consultivos coordenados
...
- Evidencia normativa:
  ...
  - `per_dcomp-web_-informar-debitos-para-compensacao.pdf`
- Risco:
  - Um unico booleano de bloqueio pode esconder se o problema e meio incorreto, credito vedado, debito vedado, status processual, qualidade de importacao ou SELIC indisponivel.
- Diretriz:
  - Retornar resultados consultivos separados e explicaveis por camada.
